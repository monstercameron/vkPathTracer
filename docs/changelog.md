...
## 2026-05-09 (session 44)

### SYSTEM.md contract maturity sweep, Lua scripting hardening, asset reorganization

- `b3678ed` scripting: `reload_bindings` now reuses the cached `LuaStatePool`, bytecode cache, command queue, variable snapshots, and runtime states when the per-binding `(entity, source, language, entry, module_id, enabled)` identity tuple is unchanged across reloads — avoids the `lua_close` fault on long-lived state from a prior Play session and skips a destructive O(scripts) clear on the play→edit→play toggle. `m_disabled_until_reload` is hoisted out of the identity-keep branch so reload always re-enables budget-killed scripts (the contract `pt_script_dispatch_contract_smoke` checks). `qtRuntime` `OnDisable` dispatch wrapped in try/catch, runtime world cache reset on mode exit so the next `OnLoad` doesn't run against stale entity pointers.
- `8116e12` scripting: standardized binding contracts. 26 bindings now `luaL_argerror` on type mismatch (programming errors fail fast); missing-object cases (entity destroyed, component absent, audio system unavailable) keep returning `nil` (recoverable); number args reject non-finite via `std::isfinite`; `world.entity` ≡ `world.find_entity` alias removed; six ASCII-banner doc blocks describe per-category contracts. Adds a 50 ms per-script-per-hook wall-clock budget to `LuaInstructionBudget` — the count-hook now checks elapsed wall-clock alongside instructions and memory, raises `luaL_error("wall-clock budget exceeded")` and rides the existing disable-until-reload path. New `ScriptingStatus::wall_clock_kills_total` counter exposed via `status()`. Closes the only unfilled hardening gap from the crash-isolation audit.
- `18cf062` assets: 24 Lua scripts reorganized into `assets/scripts/{systems,user,test}/` with one-line `-- [bucket/path]` headers on every file. Five new reusable engine-shipped modules under `systems/`: `camera/orbit.lua`, `movement/character_controller.lua`, `audio/spatial_emitter.lua`, `light/animated_sun.lua`, `util/lifecycle_logger.lua`. Three new test probes under `test/`: `script_stack_overflow_probe.lua`, `script_wall_clock_probe.lua`, `script_counter_probe.lua`. Legacy root copy of `generic_fps_camera.lua` deleted (canonical lives in `systems/`).
- `a68a2aa` scenes: 26 scene JSONs + `game/scenes/relay_yard_lowest_lod_demo.json` + `tools/generate_lowest_lod_showcase_scene.ps1` rewritten to point at the new `assets/scripts/{systems,user,test}/` paths. Adds `assets/scenes/representative_acceptance_scene.json` — 88 entities meeting the SYSTEM.md acceptance-gate spec (continuous physics + camera motion + 50 active scripts + 20 audio sources). Unrelated FPS demo lighting tweaks: dimmer hero/scene fill, brighter flashlight with new beam/blend/forward-offset/drop params.
- `bc5e99f` tests: six new stability cases in `script_dispatch_contract_smoke.cpp` — malformed Lua syntax, stack overflow, 50 ms wall-clock kill (instruction_budget=1e9 so wall-clock fires first; asserts `wall_clock_kills_total++`), wrong-type binding-call survival across six representative bindings, `disabled_until_reload` clears on reload (regression test), `BindingIdentity` state continuity (counter probe ticks, identical reload preserves state, structural reload drops it). `scripting_runtime_smoke` syntax sweep switched to `recursive_directory_iterator` so it still finds every `.lua` under the new layout. Path-only updates in `observability_smoke` and `vulkan_backend_snapshot_smoke` for the script reorg and `Result<T>` return-type changes.
- `558052e` render+scene: closes SYSTEM.md hot-path audit (`:1758`). `TileScheduler::rebuild_order` replaces `reserve` + per-tile `push_back` with a pre-sized `resize` + indexed writes (single allocation per `begin_sample`, zero `push_back` on the per-sample path). `SnapshotRing::publish` mutex sites documented as guarding only the once-per-sim-tick auxiliary stats/slots/readers/determinism bookkeeping; `m_current` is already a lock-free `atomic<shared_ptr>`. The two `RenderCoordinator` `sleep_for(1ms)` sites accepted as the convergence-idle throttle and the publish-rate pacer respectively; rationale comments added at each call site.
- `17fc2a3` contracts: closes SYSTEM.md `:1409` (state-machine tables) and `:1379` (`Result<T>` convention). New full state×method grids on `IPathTracer`, `IRenderCommandContext`, `IRenderResourceAllocator`, `IShaderCompiler`, `IShaderCache`, `ISceneRuntime`, `IScriptRuntime`, `ITimeSource`, `VulkanGpuPathTracer`, `D3D12GpuPathTracer`. Existing `IJobSystem`/`IAudioSystem` grids exploded grouped column families into per-method columns. `assert_state` helper extended in `Lifecycle.h` with templated overloads gated on `std::is_enum_v` for subsystem-specific enums. `Result<T>`/`Status` conversions: `IRenderBackend::initialize`/`shutdown`, `IShaderCompiler::compile_compute_shader`, `IAudioDevice::open`/`start`, `IUiRenderer::initialize` (~9 fail-able ops). Throws converted: `CrashRecorder.cpp:80` (now `Result<string>` propagating IOError), `PhysicsWorld.cpp:707` (constructor degrades to Failed lifecycle via `CreatePhysicsWorldResult` factory). `JobSystem.cpp:621` retained — chained-job `exception_ptr` propagation contract. Predicates correctly skipped: ring `try_*`, `IShaderCache::query`, `IUiPlatformBridge::open_file_dialog`, `IWindow::initialize`/`poll_events`. `AudioSystem.cpp` also picks up an unrelated tone-generator branch for ricochet/impact/hit URIs.
- `f2cc20e` app: `qt-stress-gate` now tracks four `vkp.sim.*` metrics during stress phases (`tick_us` histogram, `tick_total`, `deadline_misses_total`, `tick_hz`); JSON artifact gains a top-level `sim` block plus three summary fields. Used to measure the new representative scene against the SYSTEM.md acceptance gates (`:1729` UI 60 Hz / p99 < 16 ms; `:1736` sim 60 Hz / < 1% jitter / 0 deadline misses). On this Windows host's debug build: `repaint_hz=11.5`, `tick_hz=37.7`, `jitter_pct=4773%`, `deadline_misses_total=443` — both gates fail; SYSTEM.md records the actual measurement and leaves the boxes unchecked pending a release-build re-run.

### LFS migration, host-context heap-allocation crash fix, end-to-end perf optimization sweep (session 44 cont.)

- `64fc639` fix(scripting): heap-allocate `LuaHostContext` per `lua_State` instead of stack-allocating per dispatch and stamping the registry with the pointer. Diagnosed a crash on the FPS demo `play→edit→play` sequence: the BindingIdentity hot-reload optimization reuses the lua_State across dispatches, so any path that consulted `LUA_REGISTRYINDEX["vkpt.current_host_context"]` across the toggle dereferenced dead stack memory. Now `LuaStatePool::Impl::State` owns a `unique_ptr<LuaHostContext>`; the registry pointer is set once at state creation and explicitly nilled in `~State` before `lua_close`. Per-dispatch fields (world, commands, context, binding, hook, diagnostics, entity-id, pure, instruction-budget) are copied into the heap host at hook entry and `ResetHostSnapshot`'d at every exit. New regression case G in `pt_script_dispatch_contract_smoke` exercises the exact crash path (2× OnUpdate → OnDisable → identity-matching reload → OnLoad → OnUpdate); the counter probe gains `on_load`/`on_disable` hooks that touch `ctx.world:find_entity` so a dangling-host regression manifests as a binding fault instead of a silent no-op.
- `0927fa5` ops: migrated the `arch/snapshot-bus` branch to Git LFS for `*.glb,*.bin,*.fbx,*.obj,*.exr,*.hdr,*.zip` — 229 commits rewritten, 45 paths now LFS-tracked, 1.8 GB of binary content moved out of pack files. Unblocks GitHub's 100 MB-per-file limit (the relay-yard LOD pack had a 105 MB `Meshy_AI_Rusty_5000W_Portable__0507153845_texture.glb`). `main` history untouched (`--include-ref=refs/heads/arch/snapshot-bus`).
- `913f1af` perf: SYSTEM.md-aligned hot-path sweep across script dispatch, render coordinator, snapshot bus, and job system. **Script** (`ScriptRuntime.cpp`): cached entity prototype + ctx skeleton per `lua_State` and reuse via `{__index=prototype}` metatables — `PushEntityObject` and `PushContextTable` no longer push 14+30 closures/setfields per dispatch; only per-dispatch value fields refresh. Inlined `PushHostClosure` (host arg was dead) at all 37 call sites. Gated `CaptureLuaScriptVariables` on `context.editor.canvas_enabled` so non-editor Play skips the per-dispatch upvalue/global stack walk. `ResetHostSnapshot` and `runtime_state` field clears collapse to aggregate-init memcpys. **Render** (`RenderCoordinator.cpp`, `TileScheduler.cpp`): removed `m_statsMutex` from the per-tile hot path — stack-local shadow accumulators (per-GPU tile counts, latencies, snapshot first-tile, gen-lag) updated lock-free per tile and folded into `m_stats` once per sample. FNV-1a fingerprint short-circuit on `TileScheduler::rebuild_order` skips the `iota`+`stable_sort` when sample-to-sample inputs are identical. All-dirty / all-clean specialized comparators dispatched from a pre-scan. **Scene** (`SceneSnapshot.cpp`, `WorldCommandBuffer.cpp`): skip `BuildInstanceMotion` when topology unchanged AND COW instances array shares storage with previous — empty motion is provably correct, saves an O(n) walk plus the motion CowArray equality scan. Material-dependent COW reuse consolidated. `WorldCommandBuffer` first-frame `reserve(64)`. **Jobs** (`JobSystem.cpp`): replaced 1 ms CV polling with predicate-based wait + 100 ms safety net; cache-line-padded `JobState::pending` with `alignas(hardware_destructive_interference_size)` to stop ping-ponging with adjacent `mutex`/`cv`; `create_group(0)` shims no longer insert into `m_jobs` (plugs SYSTEM.md §6.5 unbounded-growth case); shutdown `dropped_count` is now a real count instead of hardcoded 0; `m_workersBusy` `fetch_add/sub` relaxed (status() consumers use acquire-on-load). **Measured** on representative_acceptance_scene gate (debug build, 30 s run): `lua_motion` `scene.snapshot_build_us` sum −41.7 %, `scene.publish_us` sum −34.4 %, p50 24→12 µs (−50 %); `camera_pan` snapshot_build sum −18 %, publish sum −10 %; `idle` `vkp.qt.scripts_us` sum −45 %. 8/8 smokes green.
- `f955ef8` perf(ui): cached the four hot filesystem helpers behind `BuildQtAssetBrowserDock` — 28 scene-JSON full parses + a recursive 88-file `game/models/lods` walk + 8× `exists()` per row + `weakly_canonical()` per `samepath` were happening on every cold dock rebuild. `QtDockFindAssetFiles` is now TTL-cached (10 s); `QtDockReadSceneDisplayName` reads the first 8 KiB and string-searches for `"scene_name"` instead of full `JsonParser::parse`; `QtDockFindRepoRelativePath`, `QtDockAbsoluteNormalizedPath`, `QtDockDisplayPath` are all memoized. Per-sub-builder histograms (`vkp.ui.dock_<name>_us`) added so future regressions surface per panel. **Measured**: `vkp.ui.preview_dock_us` max during `game_mode_walk` 675803 → 112953 µs (−83 %, 6× faster). `sim_observed_hz` 36.97 → 37.86 (+2.4 %), `jitter_pct` 4183 → 3943 (−5.7 %), `max_tick_us` 713896 → 673924 (−5.6 %). Residual `max_tick_us` is now in the cold first-time `qtStressEnterGameMode` path (physics sync + 50 first-time `on_load` dispatches), paid once per session; subsequent re-entries benefit from `b3678ed`'s BindingIdentity reuse.

## 2026-05-08 (session 43)
