-- Compatibility path for scenes authored before assets/scripts/systems/.
-- The current sandbox disables require/dofile/loadfile, so this file falls back
-- to a local copy until the Lua loader grows safe include support.
local system_source = "assets/scripts/systems/generic_fps_camera.lua"
if type(include) == "function" then
  local ok, loaded = pcall(include, system_source)
